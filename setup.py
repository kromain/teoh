...
if __name__ == '__main__':
    ...
    if sys.argv[-1] in package_platforms:
        ...
        print("[INFO] Generating HTML documentation in doc/_build/html ...")
        cwd = os.getcwd()
        os.environ["PYTHONPATH"] = cwd
        # This updates the .rst files with all the found packages and modules
        # autosummary creates wrong links right if there are .rst files in the folder, so remove autogenerated ones
        for rst_file in (glob.glob("doc/skynet.*.rst") + glob.glob("doc/modules.rst")):
            os.remove(rst_file)
        subprocess.call(["sphinx-apidoc", "-f", "-o", "doc", "skynet"], shell=use_shell)
        ...
        subprocess.call(["make", "html"], shell=use_shell)
        os.chdir(cwd)
