...
function parseArgs(){
    var options = {
        ...
        driver: {abbr: "d", required: true, help: "Driver to use (inspector or dart).", choices: ["inspector","dart"]},
...
function skynetRun(){
    ...
    SkyNet.connectToRemote(opts.target, opts.driver).then(runMocha).then(SkyNet.disconnect.bind(SkyNet));
}
